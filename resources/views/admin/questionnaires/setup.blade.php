<x-admin-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Atur Sub-bab & Pertanyaan') }}
        </h2>
    </x-slot>

    <div class="py-12">
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('admin.questionnaires.store') }}" id="questionnaire-form">
                @csrf
                ...
                <input type="hidden" name="estimated_duration" value="{{ $data['estimated_duration'] }}">
                <input type="hidden" name="is_active" value="{{ $data['is_active'] ?? 0 }}">
                ...
                <div class="bg-gradient-to-r from-indigo-900 via-indigo-800 to-indigo-700 shadow-2xl rounded-2xl p-8 mb-12 text-white">
                    ...
                    <div class="grid grid-cols-3 gap-4">
                        <div class="bg-white bg-opacity-10 backdrop-blur-sm rounded-lg p-4">
                            ...
                            <p class="text-lg font-semibold">{{ $faculties->firstWhere('id', $data['faculty_id'])->name ?? '-' }}</p>
                        </div>
                        <div class="bg-white bg-opacity-10 backdrop-blur-sm rounded-lg p-4">
                            <p class="text-indigo-100 text-xs uppercase tracking-wide mb-1">Estimasi Durasi</p>
                            <p class="text-lg font-semibold">{{ $data['estimated_duration'] ?? 'N/A' }} Menit</p>
                        </div>
                        <div class="bg-white bg-opacity-10 backdrop-blur-sm rounded-lg p-4">
                            <p class="text-indigo-100 text-xs uppercase tracking-wide mb-1">Status</p>
                            <p class="text-lg font-semibold">{{ $data['is_active'] ? 'Aktif' : 'Tidak Aktif' }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex items-center justify-between gap-4 mt-12 pt-8 border-t border-gray-300">
                    <a href="{{ route('admin.questionnaires.create') }}" class="inline-flex items-center px-6 py-3 bg-gray-300 hover:bg-gray-400 text-gray-800 font-semibold rounded-lg transition-all duration-300 gap-2">
